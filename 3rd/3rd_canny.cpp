 ...
 */
void applySobelFilter(BitmapManager *src, BitmapManager *dst, BitmapManager *dstAtan){
    //! 各画素の計算用(x, y, total)
    int gx = 0, gy = 0, g = 0;
    auto theta = 0;
    ...
    for (int row = 1; row < src->getHeight()-1; row++) {
        for (int col = 1; col < src->getWidth()-1; col++) {
            ...
            dstAtan->setColor(row, col, theta, theta, theta);
 ...
 */
void nonMaximumSuppression(BitmapManager* srcSobel, BitmapManager* srcAtan, BitmapManager* dst) {
    ...
    for (int row = 1; row < src->getHeight()-1; row++) {
        for (int col = 1; col < src->getWidth()-1; col++) {
            // ラジアン値から度数を取り出す
            degreeTheta = srcAtan[row][col] * 180.0 / M_PI;

            // -180 ~ 180の値について、srcAtanを近似化
            if (-180.0 <= degreeTheta && degreeTheta < -157.5)       srcAtan[row][col] = dir_0_180;
            else if (-157.5 <= degreeTheta && degreeTheta < -112.5)  srcAtan[row][col] = dir_45_225;
            else if (-112.5 <= degreeTheta && degreeTheta < -67.5)   srcAtan[row][col] = dir_90_270;
            else if (-67.5 <= degreeTheta && degreeTheta < -22.5)    srcAtan[row][col] = dir_135_305;
            else if (-22.5 <= degreeTheta && degreeTheta < 22.5)     srcAtan[row][col] = dir_0_180;
            else if (22.5 <= degreeTheta && degreeTheta < 67.5)      srcAtan[row][col] = dir_45_225;
            else if (67.5 <= degreeTheta && degreeTheta < 112.5)     srcAtan[row][col] = dir_90_270;
            else if (112.5 <= degreeTheta && degreeTheta < 157.5)    srcAtan[row][col] = dir_135_305;
            else if (157.5 <= degreeTheta && degreeTheta <= 180.0)   srcAtan[row][col] = dir_0_180;
            else {
    ...
    //! 注目画素
    auto interestedPixel;

    // 勾配方向の隣接マスを確認し、極大値でなければ0を代入
    for (int row = 1; row < src->getHeight()-1; row++) {
        for (int col = 1; col < src->getWidth()-1; col++) {
            interestedPixel = srcAtan[row][col];

            switch (interestedPixel) {
                // 左右
                case dir_0_180:
                    if (srcSobel[row][col] < srcSobel[row-1][col] || srcSobel[row][col] < srcSobel[row+1][col])
                        dst[row][col] = 0;
                    break;
                // 右上がり方向
                case dir_45_225:
                    if (srcSobel[row][col] < srcSobel[row-1][col+1] || srcSobel[row][col] < srcSobel[row+1][col-1])
                        dst[row][col] = 0;
                    break;
                // 上下方向
                case dir_90_270:
                    if (srcSobel[row][col] < srcSobel[row][col-1] || srcSobel[row][col] < srcSobel[row][col+1])
                        dst[row][col] = 0;
                    break;
                // 左上がり方向
                case dir_45_225:
...
bool checkAdoptedEdge(BitmapManager* src, int row, int col, int t_upper) {
    ...
    for (int innerRow = -1; innerRow <= 1; innerRow++) {
        for (int innerCol = -1; innerCol <= 1; innerCol++) {
            if (src[row + innerRow][col + innerCol] >= t_upper)
                return true;
        }
    }
...
void hysteresisThreshold(BitmapManager* src, BitmapManager* dst, int t_upper=200, int t_lower=100) {
    // dstにsrcをコピー
    dst.copy(src);
    ...
    for (int row = 1; row < src->getHeight()-1; row++) {
        for (int col = 1; col < src->getWidth()-1; col++) {
            // 上側は輪郭として採用
            if (src[row][col] >= t_upper)
                dst[row][col] = 255;

            // 下側は不採用
            else if (src[row][col] < t_lower)
                dst[row][col] = 0;
...
void applyCanny(BitmapManager* src, BitmapManager* dst) {
    ...
    applyGaussianFilter5x5(src, dstGauss);
    applySobelFilter(dstGauss, dstSobel, dstAtan);
    nonMaximumSuppression(dstSobel, dstAtan, dstSuppression);
    hysteresisThreshold(dstSuppression, dst, t_upper, t_lower);
...
int main(int argc, char *argv[]) {
    ...
    // prewittフィルタ適用
    applyCanny(&src, &dstCanny);
    dstCanny.writeData(canny_filename);
